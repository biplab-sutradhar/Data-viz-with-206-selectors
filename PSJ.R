...
missing <- setdiff(required_objects, names(PSJ))
...
viz <- list(
  coverage = ggplot() +
    ...
    geom_segment(
      aes(x=problemStart/1e3, xend=problemEnd/1e3, y=1, yend=1),
  ...
  modelSelection = ggplot() +
    ...
    geom_segment(
      aes(min.log.lambda, as.integer(errors),
